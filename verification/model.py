...
class TransitionFunction:
    ...
    def __init__(
            self,
            transition_matrix: Union[tf.sparse.SparseTensor, tf.Tensor],
        ...
        if sparse:
            ...
            self.num_states = tf.shape(transition_matrix)[0]
        ...
        if sparse:
            ...
            self.num_actions = tf.shape(transition_matrix)[1]

        self._transitions = transition_matrix
        self.split_label_from_latent_space = split_label_from_latent_space

        if sparse:
            self.enabled_actions = tf.cast(
                tf.sparse.reduce_sum(self._transitions, axis=-1, output_is_sparse=True),
                dtype=tf.bool)
        else:
            self.enabled_actions = tf.reduce_sum(self._transitions, axis=-1)

        if assert_distribution:
            state_action_pairs = tf.sparse.reduce_sum(self._transitions, axis=-1, output_is_sparse=True)
    ...
    def to_dense(self, *args, **kwargs) -> tf.Tensor:
        ...
        """
        if self._sparse:
            return tf.sparse.to_dense(self._transitions)
        else:
            return self._transitions
    ...
    def __call__(self, latent_state: tf.Tensor, latent_action: tf.Tensor):
        ...
        """
        assert self.sparse, "This transition function does not relay on sparse tensors."\
                            "Get the transition matrix instead."
        ...
        def _get_prob_value(transition):
            ...
            if not self.sparse:
                return self._transitions[state, action, next_state]
            else:
                # check if the action has been visited in the given state
                # during the transition sparse tensor construction
                action_is_enabled = tf.squeeze(
                    tf.sparse.to_dense(
                        tf.sparse.slice(self.enabled_actions, [state, action], [1, 1])))

                if action_is_enabled:
                    probs = tf.squeeze(tf.sparse.slice(self._transitions, [state, action, next_state], [1, 1, 1]).values)
                    # if the entry of the slice is empty, then the probability is 0
                    return 0. if tf.equal(tf.size(probs), 0) else probs
                # if not, then use a backup transition function to retrieve
                # the probability distribution for [state, action]
                else:
                    if self.split_label_from_latent_space:
            ...
            if not self.sparse:
                ...
                if action_is_enabled:
                    return tf.squeeze(tf.sparse.to_dense(tf.sparse.slice(
                        self._transitions, [state[0, ...], action[0, ...], 0], [1, 1, self.num_states])))
                else:
...
class TransitionFunctionCopy(TransitionFunction):
    ...
    def __init__(
        ...
        repeated_state_space = tf.tile(latent_state_space, multiples=[num_actions, 1])
        ...
        def _get_sparse_entry(latent_state):
            ...
            if split_label_from_latent_space:
                next_latent_states = [
                    repeated_state_space[..., :atomic_prop_dims],
                    repeated_state_space[..., atomic_prop_dims:]]
            else:
                next_latent_states = [repeated_state_space]
        ...
        def _get_entry(latent_state):
            ...
            return tf.reshape(probs, (num_actions, num_states))

        @tf.function
        def _get_rows(state_action):
            state, action = state_action
            return transition_function(state[None], action[None]).prob(latent_state_space)

        if sparse and batch_mode:
            ...
            transitions = tf.map_fn(
                fn=_get_entry,
                elems=latent_state_space,
                fn_output_signature=tf.TensorSpec(shape=[num_actions, num_states]))
        else:
            transitions = tf.map_fn(
                fn=_get_rows,
                elems=(repeated_state_space, repeated_action_space),
                fn_output_signature=tf.TensorSpec(shape=(num_states,)))
            transitions = tf.reshape(transitions, (num_states, num_actions, num_states))
...
class TransitionFrequencyEstimator(TransitionFunction):
    ...
    def to_dense(self, backup_tensor: Optional[tf.Tensor] = None, *args, **kwargs) -> tf.Tensor:
        ...
        """
        dense_tensor = tf.sparse.to_dense(self._transitions)
