import collections
from typing import Callable, Optional, Union, OrderedDict
...
class LatentEmbeddingTFEnvironmentWrapper(TFEnvironment):
    def __init__(
            ...
            number_of_discrete_actions: int,
            reward_scaling: Optional[float] = 1.,
            latent_state_dtype: Optional[tf.dtypes.DType] = tf.int32
    ):
    ...
    def _reset(self):
        ...
        if type(time_step.observation) is OrderedDict:
            time_step = time_step._replace(observation=collections.OrderedDict([
                (key,  tf.cast(value, tf.float32)) for key, value in time_step.observation.items()
            ]))
    ...
    def wrap_latent_policy(self, latent_policy: TFPolicy, observation_dtype: Optional[tf.dtypes.DType] = None):

        class LatentPolicyWrapper(TFPolicy):

            _latent_policy = latent_policy
            _observation_dtype = latent_policy.time_step_spec.observation.dtype \
                if observation_dtype is None else observation_dtype
